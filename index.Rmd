...
```{r week 12, echo=FALSE, fig.show="hold", out.width="50%"}
...
femkemetal <-
  get_playlist_audio_features("bnfcollection", "0xKB9peSIoqMIexrmubkV3") %>%
  add_audio_analysis() %>%
  mutate(
  ...
  unnest(cols = c(pitches, timbre)) %>% slice_sample(n = 15)
spotifymetal <-
  get_playlist_audio_features("bnfcollection", "37i9dQZF1DX9qNs32fujYe") %>%
  add_audio_analysis() %>%
  mutate(
    segments = map2(segments, key, compmus_c_transpose),
    pitches =
...
metal <-
  bind_rows(
    femkemetal %>% mutate(playlist = "Femke's metal") %>% slice_head(n = 15),
    spotifymetal %>% mutate(playlist = "Spotify's metal") %>% slice_head(n = 15)
  ) 
...
metal_juice <-
  ...
  prep(metal %>% mutate(track.name = str_trunc(track.name, 30))) %>%
  juice() %>%
  column_to_rownames("track.name")

metal_dist <- dist(metal_juice, method = "euclidean")
...
knitr::include_graphics("tempogramblacklist.png")

knitr::include_graphics("tempogramarchangel.png")
...
Painkiller %>%
  ...
  scale_fill_viridis_c(guide = "none") +
  theme_classic() +
  labs(x = "", y = "") + ggtitle("Cepstrogram of 'Painkiller' by Judas Priest")
